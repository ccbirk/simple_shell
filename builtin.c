 ...
 * @info: the struct containing potential arguments and
 * is used to maintain constant function prototype.
 * Return: this exits with a given exit status (0)
 * if info.argv[0] != "exit"
 */
int _myexit(info_t *info)
{
	...
	if (info->argv[1]) /* If the is an exit argument */
	{
...
int _mycd(info_t *info)
{
	...
	else
	{
		...
		_setenv(info, "PWD", getscwd(buffer, 1024));
	}
	return (0);
}
